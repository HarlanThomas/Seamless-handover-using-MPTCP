...
%                     fprintf ('cross\n');
                case 0    % 在没有跨区的情况下
                    if  (final_score(best_net) / final_score(cur_ID) >  1.08 )  
                        is_handover = 1;
                        cur_ID = best_net; 
                    else
                        is_handover = 0;
...
%   FAHP + TOPSIS
    % 目前只考虑一种业务
        ww = [0.2 0.2 0.1 0.15 0.1 0.25];
        topsis_v = topsis_r;
        for k = 1:6
            % 视频业务 由于 Wimax 的偏激，最后特殊化向量[0.1 0.15 0.15 0.25 0.1 0.25]
            topsis_v(:, k) = topsis_v(:, k) * w_3(k);
        end
        A_plus = ones(1, 6);
        A_minus = zeros(1, 6);
%         A_plus = max(topsis_v);
...
%                     fprintf ('cross\n');
                case 0    % 在没有跨区的情况下
                    if  (topsis_c(topsis_best_net) / topsis_c(topsis_cur_ID) >  1.08 )  
                        topsis_is_handover = 1;
                        topsis_cur_ID = topsis_best_net;   
                    end
                    topsis_handoverNUM = topsis_handoverNUM + topsis_is_handover;
...
%   different scenario
%      save('maj_r_ctmat_s2','count_matrix');
%      save('maj_r_RSSmat','RSS_count_test');


%% 保存切换次数的矩阵
% % % % % % % % %      hd_numv0 = showhandover;
% % % % % % % % % %      save(['hd_num_v',num2str(velocity)],['hd_numv',num2str(velocity)]);
% % % % % % % % %      save(['hd_num_v0'],['hd_numv0']);
